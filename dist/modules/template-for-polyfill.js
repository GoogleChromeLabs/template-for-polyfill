...
(() => {
    ...
    if ('htmlFor' in HTMLTemplateElement.prototype)
        return;
    ...
    const processTemplate = (template) => {
        ...
        while ((node = walker.nextNode())) {
            ...
            if (node.nodeType === Node.COMMENT_NODE) {
                ...
                processingInstructionText = `?${node.target.toLowerCase()}`;
                if (node.data)
                    processingInstructionText = `${processingInstructionText} ${node.data}`;
            }
            ...
            else if (processingInstructionText.toLowerCase().startsWith('?end') &&
                startNode) {
